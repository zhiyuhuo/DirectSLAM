...
int main(int argc, char** argv)
{
    ...
    int kn = 0;
    int sn = 0;
    std::string rootSaveDir = "/home/rokid/Pictures/patent/";
    ...
    while (true) {
        ...
        if (cmd == 's') {
            ...
            if (started) {
                ...
                if (nImg % 12 == 0) {
                    std::string imgName = rootSaveDir + "K" + std::to_string(kn) + ".png";
                    cv::imwrite(imgName.c_str(), img_input);
                    cv::Mat sbi;
                    cv::resize(img_input, sbi, cv::Size(40, 30));
                    std::string imgSBIName = rootSaveDir + "Ksbi" + std::to_string(kn) + ".png";
                    cv::imwrite(imgSBIName.c_str(), sbi);
                    kn++;
                }
                else if (nImg % 4 == 0)
                {
                    std::string imgName = rootSaveDir + "S" + std::to_string(sn) + ".png";
                    cv::imwrite(imgName.c_str(), img_input);
                    cv::Mat sbi;
                    cv::resize(img_input, sbi, cv::Size(40, 30));
                    std::string imgSBIName = rootSaveDir + "Ssbi" + std::to_string(sn) + ".png";
                    cv::imwrite(imgSBIName.c_str(), sbi);
                    sn++;
                }
